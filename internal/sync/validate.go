...
func validateChecksum(restOp *utils.RestOp) bool {
	...
	if restOp.Model == "Pool" {
		...
		utils.AviLog.Infof("Checksum is the same for pool object with name %s", restOp.ObjName)
		...
		utils.AviLog.Infof("Checksum is the same for vs object with name %s", restOp.ObjName)
	} else {
